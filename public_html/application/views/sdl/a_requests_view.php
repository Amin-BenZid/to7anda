   ...
   if ($req->etat == 1) { 
    ...
    $ch.=$req->id.'/////'.$req->id_appa.'/////'.$req->id_prop.'/////'.$req->id_tag.'/////'.$req->cin_pass;
    $ch.='" onclick="acceptation_auto(this.id);"><i class="fa fa-check"></i></button>';
    }
    $ch.='<button type="button" class="btn btn-default" title="Annulation" id="';
    $ch.=$req->id.'/////'.$req->id_appa.'/////'.$req->id_prop.'/////'.$req->id_tag.'/////'.$req->cin_pass;
    $ch.='" onclick="annulation_auto(this.id);"><i class="fa fa-times"></i></button>';
 if ($req->etat == 4) { 

    $ch.='<button type="button" class="btn btn-default" title="Termination" id="';
    $ch.=$req->id.'/////'.$req->id_appa.'/////'.$req->id_prop.'/////'.$req->id_tag.'/////'.$req->cin_pass;
    $ch.='" onclick="termination_auto(this.id);"><i class="fa fa-certificate"></i></button>';
 }

    $ch.='<button type="button" class="btn btn-default" title="Tag" id="';
    $ch.=$req->id.'/////'.$req->id_appa.'/////'.$req->id_prop.'/////'.$req->id_tag.'/////'.$req->cin_pass;
    $ch.='" onclick="tag_auto(this.id);"><i class="fa fa-credit-card"></i></button>';
    echo $ch;
